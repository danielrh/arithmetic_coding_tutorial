...
void encode_with_adaptive_probability() {
    ...
    for (size_t i = 0; i < sizeof(uncompressed); ++i) {
        ...
        for(int bit = 128; bit > 0; bit >>= 1) {
            ...
            vpx_write(&wri, cur_bit, encode[i % 2][encoded_so_far + bit].prob);
            encode[i % 2][encoded_so_far + bit].record_bit(cur_bit);
    ...
    for (size_t i = 0; i < sizeof(roundtrip); ++i) {
        ...
        for(int bit = 128; bit > 0; bit >>= 1) {
            if (vpx_read(&rea, decode[i % 2][decoded_so_far + bit].prob)) {
                roundtrip[i] |= bit;
                decode[i % 2][decoded_so_far + bit].record_bit(true);
                decoded_so_far |= bit;
            } else {
                decode[i % 2][decoded_so_far + bit].record_bit(false);
            }
        }
    }
